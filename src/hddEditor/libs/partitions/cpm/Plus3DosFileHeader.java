 ...
 * 
 * Every valid file +3 dos file has a header of these in the first 128 bytes of the CPM file. 
 * It provides information such at the proper byte-level file size (CPM has file sizes in 128 bytes)
 * The filesize in the header includes the header in its size. 
 * 
 * In addition BASIC has its own header so it can tell what type of file of it. 
 * It also has a file size. This DOES NOT include the header. Its the size of the file as basic sees it. 
 * As such FileLength in the +3 basic header is always 128 bytes lower than the one in the PLUS3DOS header. 
 * 
 * Format of the header is:
 * 
 * 0-7 	   PLUS3DOS
 * 8       Soft EOF character provided when reading and the file runs out. (1A on the +3)
 * 9       Issue number (1)
 * 10      Version number (0)
 * 11-14   Filesize including the header length - LSB first
 * 15-22   BASIC header (See below)
 * 23-126  Reserved (0)
 * 127     Checksum byte
 * 
 * Checksum is simply the modulus of the bytes from 0-126 
 * Easily generated by z80 by just adding all the bytes together and ignoring any carry's  
 *  
 * 
 *BASIC header (bytes 15-22)
