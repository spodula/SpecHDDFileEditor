	...
	protected void DoExtractAllFiles() {
		...
		if (result != null) {
			...
			for (TableItem t : DirectoryListing.getItems()) {
				DirectoryEntry entry = (DirectoryEntry) t.getData();
				try {
					File TargetFilename = new File(directory, entry.filename().trim());
					...
					System.out.println("Error extracting " + entry.filename().trim() + ": " + e.getMessage());
					e.printStackTrace();
				}
