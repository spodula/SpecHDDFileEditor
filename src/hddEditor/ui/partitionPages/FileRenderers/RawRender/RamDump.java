	...
	protected void doExtractFiles() {
		...
		if (result != null) {
			...
			if (IYReg == 0x5c3a) {
				...
				int PROG = (ram[0x5c54 - diff] & 0xff) * 256 + (ram[0x5c53 - diff] & 0xff);
				int VARS = (ram[0x5c4c - diff] & 0xff) * 256 + (ram[0x5c4b - diff] & 0xff);
				int E_LINE = (ram[0x5c5a - diff] & 0xff) * 256 + (ram[0x5c59 - diff] & 0xff);
				int RAMTOP = (ram[0x5cb3 - diff] & 0xff) * 256 + (ram[0x5cb2 - diff] & 0xff);

				EndOfAnyBasic = RAMTOP - diff;
				// some basic checking.
				if ((VARS > PROG) && (E_LINE > VARS) && (PROG > 23754) && (PROG < 25000)) {
					byte BasicData[] = new byte[E_LINE - PROG];
					System.arraycopy(ram, PROG - diff, BasicData, 0, BasicData.length);
